...
**White-Label Core** - A stripped-down, generic version of Actual Budget that maintains the core infrastructure (CRDT sync, file management, authentication, rules engine) while removing all budget-specific features. This white-label foundation can be extended to build domain-specific applications (e.g., clinic session annotation, inventory management, etc.).

**Original Project**: Actual Budget (https://github.com/actualbudget/actual) - A local-first personal finance tool written in TypeScript/JavaScript.
...
The codebase has been transformed into a white-label core with:

**✅ Retained Core Features:**

- CRDT synchronization system (conflict-free replicated data types)
- File management (renamed from "budget files" to "files")
- Authentication system (password + OpenID Connect)
- Generic rules engine (without budget-specific fields)
- Multi-user support
- Sync server infrastructure

**❌ Removed Budget-Specific Features:**

- Accounts, Categories, Transactions
- Bank sync integrations (GoCardless, SimpleFin, PluggAI)
- Budget types, budget pages, reports
- Payees, Schedules, Tags
- All budget-specific UI components

**📁 Current Database Schema:**

- `messages_crdt` - CRDT synchronization messages
- `messages_clock` - Logical clock for CRDT
- `__migrations__` - Migration tracking
- `__meta__` - Metadata storage
- `rules` - Generic rules engine
- `preferences` - Synced preferences

All budget-specific tables have been removed. The database starts fresh with a single `init.sql` migration file.
...
**Important**: When creating a new entity type in the white-label application, follow the comprehensive guide in **[CREATING_ENTITIES.md](./CREATING_ENTITIES.md)**.

This guide covers:
- Database schema creation
- AQL schema definition
- Type definitions
- **Entity type registration for rules system** (required for rules support)
- Server handlers
- Redux slices
- UI components
- Complete checklist and best practices

The entity creation process includes registering your entity type with the rules system so that users can create rules that operate on your entity type with dynamic field validation.
