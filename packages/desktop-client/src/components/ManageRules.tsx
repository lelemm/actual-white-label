...
export function ruleToString(rule: RuleEntity, data: FilterData) {
  ...
  const actions = rule.actions.flatMap(action => {
    ...
    } else if (action.op === 'delete-entity') {
      return [friendlyOp(action.op), '(delete)'];
    } else {
      return [];
    }
  });
  const entityTypeLabel = rule.entityType ? `[${rule.entityType}] ` : '';
  return (
    entityTypeLabel +
...
export function ManageRules({
  ...
  // White-label version: Removed hooks for deleted entities
  ...
  const filteredRules = useMemo(() => {
    // White-label version: Schedule filtering removed
  ...
  async function loadRules() {
    ...
    // White-label version: payees-get-rules removed
  ...
  const init = useEffectEvent(() => {
    async function loadData() {
      await loadRules();
      setLoading(false);
      // White-label version: getPayees removed
    }
  ...
  function onCreateRule() {
    const rule: NewRuleEntity = {
      entityType: '', // Will be set by user in RuleEditor
      stage: null,
      conditionsOp: 'and',
      conditions: [
        {
