...
function FileMenu({ onDelete, onClose, onDuplicate }: FileMenuProps) {
...
function FileMenuButton({ onDelete, onDuplicate }: FileMenuButtonProps) {
  const { t } = useTranslation();
...
function FileState({ file, currentUserId }: FileStateProps) {
  const { t } = useTranslation();
...
type FileListItemProps = ComponentPropsWithoutRef<typeof GridListItem<File>> & {
...
export function FileSelection({
  ...
  const onSelect = async (file: File): Promise<void> => {
    ...
    if (!id) {
      if (isRemoteFile) {
        await dispatch(downloadFile({ cloudFileId: file.cloudFileId }));
      } else {
        await dispatch(loadFile({ id: file.id }));
      }
    } else if (!isRemoteFile && file.id !== id) {
      await dispatch(closeAndLoadFile({ fileId: file.id }));
    } else if (isRemoteFile) {
      await dispatch(closeAndDownloadFile({ cloudFileId: file.cloudFileId }));
    }
  };
  ...
  return (
    ...
    >
      ...
      <FileList
        ...
        onDelete={(file: File) =>
          dispatch(
            pushModal({ modal: { name: 'delete-file', options: { file } } }),
          )
        }
      ...
      {!quickSwitchMode && (
        ...
        >
          <Button
            ...
            onPress={() => {
              // Import functionality removed in white-label version
            }}
          >
