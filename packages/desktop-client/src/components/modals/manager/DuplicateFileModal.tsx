...
import { duplicateFile } from '@desktop-client/files/filesSlice';
...
export function DuplicateFileModal({
  ...
  useEffect(() => {
    (async () => {
      setNewName(await uniqueFileName(file.name + fileEndingTranslation));
    })();
  }, [file.name, fileEndingTranslation]);

  const validateAndSetName = async (name: string) => {
    const trimmedName = name.trim();
    const { valid, message } = await validateFileName(trimmedName);
    if (valid) {
...
async function validateFileName(name: string): Promise<{
  valid: boolean;
  message?: string;
}> {
  return send('validate-file-name', { name });
}

async function uniqueFileName(name: string): Promise<string> {
  return send('unique-file-name', { name });
}
