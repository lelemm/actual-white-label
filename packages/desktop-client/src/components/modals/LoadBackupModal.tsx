...
import { type Backup } from 'loot-core/server/files/backups';
...
import { loadBackup, makeBackup } from '@desktop-client/files/filesSlice';
...
export function LoadBackupModal({
  ...
  const [prefsFileId] = useMetadataPref('id');
  const fileIdToLoad = fileId ?? prefsFileId;

  useEffect(() => {
    if (fileIdToLoad) {
      send('backups-get', { id: fileIdToLoad }).then(setBackups);
    }
  }, [fileIdToLoad]);
              ...
              {latestBackup ? (
                <Block>
                  ...
                  <Button
                    variant="primary"
                    onPress={() => {
                      if (fileIdToLoad && latestBackup.id) {
                        dispatch(
                          loadBackup({
                            fileId: fileIdToLoad,
            ...
            {previousBackups.length === 0 ? (
              ...
              <BackupTable
                ...
                onSelect={id => {
                  if (fileIdToLoad && id) {
                    dispatch(
                      loadBackup({ fileId: fileIdToLoad, backupId: id }),
                    );
                  }
                }}
              />
