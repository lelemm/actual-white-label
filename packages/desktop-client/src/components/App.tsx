...
// BudgetMonthCountProvider removed for white-label version
...
import { closeFile, loadFile } from '@desktop-client/files/filesSlice';
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Open the last opened file, if any
      dispatch(
        setAppState({
          loadingText: t('Opening last file...'),
        }),
      );
      const fileId = await send('get-last-opened-backup');
      if (fileId) {
  ...
  return fileId ? <FinancesApp /> : <ManagementApp />;
}
