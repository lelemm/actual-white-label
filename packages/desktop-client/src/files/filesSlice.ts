...
export const loadFiles = createAppAsyncThunk(
  `${sliceName}/loadFiles`,
  async (_, { dispatch }) => {
    const files = (await send('get-files')) as File[];
...
type ImportFilePayload = {
  filepath: string;
  // type removed - budget-specific imports removed in white-label
};

export const importFile = createAppAsyncThunk(
  `${sliceName}/importFile`,
  async ({ filepath }: ImportFilePayload, { dispatch }) => {
    // Budget-specific import removed in white-label version
    // const { error } = await send('import-file', { filepath, type });
    // if (error) {
...
const filesSlice = createSlice({
  ...
  reducers: {
    setFiles(state, action: PayloadAction<SetFilesPayload>) {
      ...
      state.allFiles = reconcileFiles(action.payload.files, state.remoteFiles);
    },
    setRemoteFiles(state, action: PayloadAction<SetRemoteFilesPayload>) {
      state.remoteFiles = action.payload.remoteFiles;
      state.allFiles = reconcileFiles(state.files, action.payload.remoteFiles);
    },
...
function reconcileFiles(
  ...
  const files = localFiles.map((localFile): File & { deleted: boolean } => {
    const cloudFileId = 'cloudFileId' in localFile ? localFile.cloudFileId : undefined;
    const groupId = 'groupId' in localFile ? localFile.groupId : undefined;
    if (cloudFileId && groupId) {
      ...
      const remote = remoteFiles.find(f => cloudFileId === f.fileId);
      if (remote) {
        // Mark reconciled
        reconciled.add(remote.fileId);

        if (remote.groupId === groupId) {
          return {
            ...localFile,
            cloudFileId,
            groupId,
