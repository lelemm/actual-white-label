...
export async function exportBuffer() {
  const { id, fileName } = prefs.getPrefs();
  if (!fileName) {
    return null;
  }
  ...
  await runMutator(async () => {
    ...
    const meta = JSON.parse(
      await fs.readFile(fs.join(fileDir, 'metadata.json')),
    );
...
export async function importBuffer(fileData, buffer) {
  ...
  const fileDir = fs.getFileDir(meta.id);

  if (await fs.exists(fileDir)) {
    // Don't remove the directory so that backups are retained
    const dbFile = fs.join(fileDir, 'db.sqlite');
    const metaFile = fs.join(fileDir, 'metadata.json');
  ...
  await fs.writeFile(fs.join(fileDir, 'db.sqlite'), dbContent);
  await fs.writeFile(fs.join(fileDir, 'metadata.json'), JSON.stringify(meta));
