...
async function getBackups(id: string): Promise<BackupWithDate[]> {
  ...
  const backupDir = fs.join(fileDir, 'backups');
...
async function getLatestBackup(id: string): Promise<LatestBackup | null> {
  const fileDir = fs.getFileDir(id);
  if (await fs.exists(fs.join(fileDir, LATEST_BACKUP_FILENAME))) {
    return {
...
export async function makeBackup(id: string) {
  ...
  if (await fs.exists(fs.join(fileDir, LATEST_BACKUP_FILENAME))) {
    await fs.removeFile(fs.join(fs.getFileDir(id), LATEST_BACKUP_FILENAME));
  }

  const backupId = `${uuidv4()}.sqlite`;
  const backupPath = fs.join(fileDir, 'backups', backupId);

  if (!(await fs.exists(fs.join(fileDir, 'backups')))) {
    await fs.mkdir(fs.join(fileDir, 'backups'));
  }

  await fs.copyFile(fs.join(fileDir, 'db.sqlite'), backupPath);
  ...
  for (const id of toRemove) {
    await fs.removeFile(fs.join(fileDir, 'backups', id));
  }
...
export async function loadBackup(id: string, backupId: string) {
  ...
  if (!(await fs.exists(fs.join(fileDir, LATEST_BACKUP_FILENAME)))) {
    ...
    await fs.copyFile(
      ...
      fs.join(fileDir, LATEST_BACKUP_FILENAME),
    );

    await fs.copyFile(
      fs.join(fileDir, 'metadata.json'),
      fs.join(fileDir, 'metadata.latest.json'),
    );
  ...
  if (backupId === LATEST_BACKUP_FILENAME) {
    ...
    await fs.removeFile(fs.join(fileDir, LATEST_BACKUP_FILENAME));
    await fs.removeFile(fs.join(fileDir, 'metadata.latest.json'));
    ...
    await fs.copyFile(
      fs.join(fileDir, 'backups', backupId),
      fs.join(fileDir, 'db.sqlite'),
    );
  }
}
