...
export class Condition {
  ...
  constructor(op, field, value, options, entityType?: string) {
    // If entityType is provided, validate against entity type registry
    // Otherwise, fall back to legacy behavior for backward compatibility
    let typeName: string | undefined;
    
    if (entityType) {
      typeName = getFieldType(entityType, field);
      assert(
        typeName,
        'invalid-field',
        `Invalid condition field: ${field} for entity type: ${entityType}`,
      );
      assert(
        isValidOp(entityType, field, op),
        'internal',
        `Invalid condition operator: ${op} (type: ${typeName}, field: ${field}, entityType: ${entityType})`,
      );
    } else {
      // Legacy mode - try to infer type from field name
      // This is for backward compatibility
      if (field === 'date') typeName = 'date';
      else if (field === 'amount') typeName = 'number';
      else if (field === 'notes') typeName = 'string';
      else typeName = 'string'; // Default fallback
    }
