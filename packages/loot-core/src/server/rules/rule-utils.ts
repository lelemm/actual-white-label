...
// White-label version: OP_SCORES without budget-specific operations
const OP_SCORES: Partial<Record<RuleConditionEntity['op'], number>> = {
  is: 10,
  isNot: 10,
  oneOf: 9,
  notOneOf: 9,
...
export function migrateIds(rule: Rule, mappings: Map<string, string>): void {
  ...
  // White-label version: Action class doesn't have type/rawValue properties
  // ID migration for actions is handled at serialization level if needed
  for (let ai = 0; ai < rule.actions.length; ai++) {
    const action = rule.actions[ai];
    // Actions are handled differently - field type is determined by entity type
    // If action.field is an ID field and action.op === 'set', we could migrate here
    // but Action class doesn't store rawValue, so this is skipped for now
  }
}
...
export function iterateIds(
  ...
  ruleiter: for (i = 0; i < rules.length; i++) {
    ...
    // White-label version: Action class doesn't have type property
    // Check if action.field matches fieldName and action.op === 'set'
    for (let ai = 0; ai < rule.actions.length; ai++) {
      const action = rule.actions[ai];
      if (action.field === fieldName && action.op === 'set') {
        if (func(rule, action.value as string)) {
          break;
        }
      }
    }
...
// White-label version: parseRecurDate removed (schedules removed)
export function parseRecurDate(desc) {
  throw new RuleError('parse-recur-date', 'Recurring dates not supported in white-label version');
}
