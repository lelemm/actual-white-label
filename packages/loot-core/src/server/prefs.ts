...
export async function loadPrefs(id?: string): Promise<MetadataPrefs> {
  ...
  const fullpath = fs.join(fs.getFileDir(id), 'metadata.json');
...
export async function savePrefs(
  ...
  if (process.env.NODE_ENV !== 'test') {
    const prefsPath = fs.join(fs.getFileDir(prefs.id), 'metadata.json');
...
export function getDefaultPrefs(id: string, fileName: string) {
  return { id, fileName };
}
