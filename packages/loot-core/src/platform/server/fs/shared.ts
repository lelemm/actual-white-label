...
export const getFileDir = id => {
  ...
  //
  // A cheesy safe guard. The id is generated from the file name,
  // so it provides an entry point for the user to accidentally (or
  // intentionally) access other parts of the system. Always
  // restrict it to only access files within the file directory by
  // never allowing slashes.
  if (id.match(/[^A-Za-z0-9\-_]/)) {
    throw new Error(
      `Invalid file id "${id}". Check the id of your file in the Advanced section of the settings page.`,
    );
  }
